<!doctype html>
<html lang="en">
  <head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <title>ReelApps</title>
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css" integrity="sha384-MCw98/SFnGE8fJT3GXwEOngsV7Zt27NXFoaoApmYm81iuXoPkFOJwJ8ERdknLPMO" crossorigin="anonymous">
    <link href="https://fonts.googleapis.com/css?family=Montserrat:300,400,600,700" rel="stylesheet">
    <link href="css/flexslider.css" rel="stylesheet">
    <link rel="stylesheet" href="style.css" />
    <link href="https://unpkg.com/aos@2.3.1/dist/aos.css" rel="stylesheet">
  </head>
  <body>
	<?php require_once('/home/u752656532/public_html/works/inc/header.php'); ?>
    <div class="container-1">
      <div class="container">
        <div class="row">
          <div class="col-lg-12">
            <h1 class="logo pt-4"><span>Reel</span>Apps</h1>
          </div>
          <div class="col-lg-4 c1-box-2">
            <h2 data-aos="fade-down"><span>Reel</span>Apps</h2>
            <h3 data-aos="fade-down">Making video marketing easier</h3>
          </div>
          <div class="col-lg-8">
            <img data-aos="fade-up" class="img-fluid" src="images/laptop.png"  />
          </div>
        </div>
      </div>
    </div>
    <div class="container-2">
      <div class="c2-box-1">
        <div class="container">
          <div class="row">
            <div class="col-lg-4 text-center" data-aos="fade-down">
                <div class="outer-circle cir-1">
                  <div class="inner-circle in-circ-1">
                    <div class="d-table">
                      <div class="d-table-cell">
                        <div class="cir-count">3 574 927</div>
                        <span></span>
                        <div class="cir-label">Users</div>
                      </div>
                    </div>
                  </div>
                </div>
            </div>
            <div class="col-lg-4 text-center" data-aos="fade-down">
                <div class="outer-circle cir-2">
                  <div class="inner-circle in-circ-2">
                    <div class="d-table">
                      <div class="d-table-cell">
                        <div class="cir-count">4 927</div>
                        <span></span>
                        <div class="cir-label">Videos Created</div>
                      </div>
                    </div>
                  </div>
                </div>
            </div>
            <div class="col-lg-4 text-center" data-aos="fade-down">
                <div class="outer-circle cir-3">
                  <div class="inner-circle in-circ-3">
                    <div class="d-table">
                      <div class="d-table-cell">
                        <div class="cir-count">3 574 927</div>
                        <span></span>
                        <div class="cir-label">Apps</div>
                      </div>
                    </div>
                  </div>
                </div>
            </div>
          </div>
        </div>
      </div>
      <div class="c2-box-2">
        <div class="container box-abouts">
          <div class="row">
            <div class="col-lg-4">
              <div class="box-about" data-aos="fade-down">
                <img src="images/ab1.jpg" />
                <h4>Easy to use</h4>
                <p>
                  It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less norma
                </p>
              </div>
            </div>
            <div class="col-lg-4">
              <div class="box-about" data-aos="fade-down">
                <img src="images/ab2.jpg" />
                <h4>Library access</h4>
                <p>
                  It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less norma
                </p>
              </div>
            </div>
            <div class="col-lg-4">
              <div class="box-about" data-aos="fade-down">
                <img src="images/ab3.jpg" />
                <h4>Built for marketing</h4>
                <p>
                  It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less norma
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="c2-box-3">
        <div class="container box-apps">
          <div class="row">
            <div class="col-lg-12 text-center">
              <h3 data-aos="zoom-in">Apps</h3>
            </div>
            <div class="col-lg-4">
              <div class="box-app" data-aos="zoom-in">
                <img src="images/app-image.png" class="img-fluid" />
                <h4><span>Video</span>Reel</h4>
                <p>It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less norma</p>
                <a href="#" class="btn btn-ra">Learn more</a>
              </div>
            </div>
            <div class="col-lg-4">
              <div class="box-app" data-aos="zoom-in">
                <img src="images/app-image.png" class="img-fluid" />
                <h4><span>Visual</span>Reel</h4>
                <p>It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less norma</p>
                <a href="#" class="btn btn-ra">Learn more</a>
              </div>
            </div>
            <div class="col-lg-4">
              <div class="box-app" data-aos="zoom-in">
                <img src="images/app-image.png" class="img-fluid" />
                <h4><span>Live</span>Reel</h4>
                <p>It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less norma</p>
                <a href="#" class="btn btn-ra">Learn more</a>
              </div>
            </div>
            <div class="col-lg-4">
              <div class="box-app" data-aos="zoom-in">
                <img src="images/app-image.png" class="img-fluid" />
                <h4><span>Clips</span>Reel</h4>
                <p>It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less norma</p>
                <a href="#" class="btn btn-ra">Learn more</a>
              </div>
            </div>
            <div class="col-lg-4">
              <div class="box-app" data-aos="zoom-in">
                <img src="images/app-image.png" class="img-fluid" />
                <h4><span>Script</span>Reel</h4>
                <p>It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less norma</p>
                <a href="#" class="btn btn-ra">Learn more</a>
              </div>
            </div>
            <div class="col-lg-4">
              <div class="box-app" data-aos="zoom-in">
                <img src="images/app-image.png" class="img-fluid" />
                <h4><span>Pixi</span>Reel</h4>
                <p>It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less norma</p>
                <a href="#" class="btn btn-ra">Learn more</a>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="c2-box-4">
          <div class="container">
            <div class="row">
              <div class="col-lg-2">
              </div>
              <div class="col-lg-8">
                <div class="flexslider" data-aos="zoom-in">
                  <ul class="slides">
                    <li>
                      <div class="ra-people">
                          <img src="images/person-1.png" class="img-fluid" />
                          <h3>
                            <span>Ginni Rometty</span> CEO, IBM
                          </h3>
                          <p>It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less norma</p>
                      </div>
                    </li>
                    <li>
                      <div class="ra-people">
                          <img src="images/person-1.png" class="img-fluid" />
                          <h3>
                            <span>Ginni Rometty</span> CEO, IBM
                          </h3>
                          <p>It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less norma</p>
                      </div>
                    </li>
                    <li>
                      <div class="ra-people">
                          <img src="images/person-1.png" class="img-fluid" />
                          <h3>
                            <span>Ginni Rometty</span> CEO, IBM
                          </h3>
                          <p>It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less norma</p>
                      </div>
                    </li>
                    <li>
                      <div class="ra-people">
                          <img src="images/person-1.png" class="img-fluid" />
                          <h3>
                            <span>Ginni Rometty</span> CEO, IBM
                          </h3>
                          <p>It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less norma</p>
                      </div>
                    </li>
                  </ul>
                </div>
                <div class="text-center cta-con">
                  <a href="#" data-aos="zoom-in" class="btn btn-cta">Call to action</a>
                </div>
              </div>
              <div class="col-lg-2">
              </div>
            </div>
          </div>
      </div>
    </div>

    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.3/umd/popper.min.js" integrity="sha384-ZMP7rVo3mIykV+2+9J3UJ46jBk0WLaUAdn689aCwoqbBJiSnjAK/l8WvCWPIPm49" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/js/bootstrap.min.js" integrity="sha384-ChfqqxuZUCnJSK3+MXmPNIyE6ZbWh2IMqE241rYiqJxyMiZ6OW/JmZQ5stwEULTy" crossorigin="anonymous"></script>
    <script src="https://unpkg.com/aos@2.3.1/dist/aos.js"></script>
    <script src="js/jquery.flexslider-min.js"></script>
    <script>
      AOS.init();

    	$(document).ready(function() {
    		$('.flexslider').flexslider({
    			animation: 'slide',
    			prevText: "",
    			nextText: ''
    		});
    	});
    </script>
  </body>
</html>
